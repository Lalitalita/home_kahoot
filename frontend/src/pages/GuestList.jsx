  ...
  return (
    <div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        <h1 className="text-2xl font-semibold text-stone-900">Qui sera là ?</h1>

        {loading && <p className="text-stone-400">Chargement...</p>}
        {error && <p className="text-rose-600">{error}</p>}
        {!loading && guests.length === 0 && (
          <p className="text-stone-400">La liste des invités n'a pas encore été publiée.</p>
        )}

        <div className="grid sm:grid-cols-2 gap-4">
          {guests.map((g) => (
            <div key={g.id} className="card flex gap-4 items-start">
              <div className="w-14 h-14 rounded-full overflow-hidden bg-stone-100 shrink-0 flex items-center justify-center text-xl">
                {g.photo_url ? (
